...
mongo_user = myclient["userMp3"]
mongo_storage = mongo_user["storage"]
...
@server.route("/proccessed", methods=["POST"])
def proccessed():
    access, err = validate.token(request)
    if err: 
        return err
    ...
    if access["admin"]:
        ...
        try:
            q = { "username": username }
            result = mongo_storage.find(q)
            return tuple([doc["mp3_fid"] for doc in result])
